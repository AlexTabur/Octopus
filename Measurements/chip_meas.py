...
#                    dpg.add_input_text(label="", tag="wl_report_file_auto", width=250, default_value="report_ic.csv")
            ...
            with dpg.group():
                ...
                dpg.add_combo(tag="chip_select", width=125, items=context.chip_list, default_value=context.chip_list[0],  callback=self.chip_select_callback)
                dpg.add_spacer(height=10)
                ...
                dpg.add_spacer(height=10)
                dpg.add_text(default_value=f"Расстояние между каналами: {context.chip_chans_dy} мкм", tag='chip_chans_dy')
                dpg.add_spacer(height=10)
                dpg.add_text(default_value=f"Расстояние между чипами: {context.chip_chans_dy} мкм", tag='chips_dy')
                dpg.add_spacer(height=200)
                dpg.add_image_button(context.axis[1]['txt_p'], pos=(1350, 100), tag="strip_move_up", callback=self.strip_move_updown, user_data=1)
                dpg.add_image_button(context.axis[1]['txt_m'], pos=(1350, 220), tag="strip_move_down", callback=self.strip_move_updown, user_data=2)
    ...
    def display_aiming_details(self, width, height):
        ...
        for i in range(context.aim_steps):
            ...
            dpg.add_text(f'{context.aim_y_width[i]}x{context.aim_z_step[i]*context.aim_z_count[i]} ({context.aim_y_width[i]*NM_PER_STEP_Y/1000}x'
    ...
    def input_area_width_um_callback(self, sender, app_data, user_data):
        val = context.gui_hlp.check_edit_data(sender, float, MAX_AREA_SIZE*NM_PER_STEP_Y/1000, MIN_AREA_SIZE*NM_PER_STEP_Y/1000)
    ...
    def input_area_height_um_callback(self, sender, app_data, user_data):
        val = context.gui_hlp.check_edit_data(sender, float, MAX_AREA_SIZE*NM_PER_STEP_Z/1000, MIN_AREA_SIZE*NM_PER_STEP_Z/1000)
    ...
    def prepare_table(self):
        ...
        with dpg.child_window(height=500, width=700, tag="tg"):
            with dpg.group(horizontal=True):
                ...
                with dpg.table(header_row=True, resizable=False, policy=dpg.mvTable_SizingStretchProp, width=self.CHIP_TABLE_WDTH,
                                ...
                                if i < context.chip_chans_left:
                                    with dpg.group(horizontal=True):
                                        ...
                                        dpg.add_image_button('aiming', show=True, user_data=i|LEFT_CHAN,
                                                             callback=self.show_aiming_res, tag='charts_chan_left' + str(i))
...
#                                                     callback=self.test_turn, user_data=i | RIGHT_CHAN)
                                ...
                                if i < context.chip_chans_right:
                                    with dpg.group(horizontal=True):
                                        ...
                                        dpg.add_image_button("aiming", show=True, user_data=i | RIGHT_CHAN,
                                                             callback=self.show_aiming_res, tag="charts_chan_right" + str(i))
                ...
                with dpg.group():
                    dpg.add_image('chan_pos_right', show=True, tag='pos_chan_right', pos=(self.CHIP_TABLE_WDTH+70,30))
    ...
    def set_act_chan_callback(self, sender, app_data, user_data):
        context.gui_hlp.confirm_msg(message='Установить позицию напортив канала как текущую? ', function=self.set_act_chan_dlg, args=user_data)
    ...
    def set_act_chan(self, *args):
        ...
        if chan & LEFT_CHAN:  # left channel
            ...
            if context.chip_ctrl.active_chan_right != -1:
                dpg.bind_item_theme("ch_num_r_" + str(context.chip_ctrl.active_chan_right), self.meas_btns_passive_theme)
    ...
    def aiming(self, *args):
        ...
        self.thread_proc = threading.Thread(target=context.pmap_procedures.aiming,
                                            args=[side, num, context.aim_steps, context.aim_y_width, context.aim_z_count,
                                                  context.aim_z_step, self.pm_dev, module, meas_chan, context.aim_x_speed], daemon=True)
    ...
    def test_turn(self, sender, app_data, user_data):
        if user_data & LEFT_CHAN:
            context.chip_ctrl.set_chan_measure(user_data&0xFF, bool(dpg.get_value("ch_test_l" + str(user_data&0xFF))), LEFT_CHAN)
        else:
            context.chip_ctrl.set_chan_measure(user_data & 0xFF, bool(dpg.get_value("ch_test_r" + str(user_data&0xFF))), RIGHT_CHAN)

    def chip_loop(self):
        def process_chanels(idx, chan, side):
            state = chan.state
            if state & CHAN_IS_CHANGED:
                if state & CHAN_IS_AIMED:
                    dpg.set_value(f"state_chan_{side}{idx}", 'А')
                    dpg.bind_item_theme(f"state_chan_{side}{idx}", self.text_state_aimed)
                else:
                    dpg.set_value(f"state_chan_{side}{idx}", '-')
                    dpg.bind_item_theme(f"state_chan_{side}{idx}", self.text_state_unaimed)
                chan.state &= ~CHAN_IS_CHANGED
