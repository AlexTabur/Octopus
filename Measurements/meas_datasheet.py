...
class DataSheet():
    ...
    def __init__(self):
        ...
        self.dt = [('ChanNum', int), ('wl', float), ('min_loss', float), ('max_loss', float), ('ITU', float), ('ITU_delt', int),
        ...
        for p in range(len(self.fin_arr)):
            for i in range(72):
                if (abs(self.fin_arr[p]['wl'] - itu[i]) < 0.41):
                    ...
                    self.fin_arr[p]['ITU_delt'] = round((self.fin_arr[p]['wl'] - itu[i]) * 1000, 0)
