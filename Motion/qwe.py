...
def place(x, y, z):
    ...
    dpg.configure_item("front_pos", p1=front_verticies[0], p2=front_verticies[1], p3=front_verticies[2],
                       p4=front_verticies[3])
    dpg.configure_item("left_pos", p1=left_verticies[0], p2=left_verticies[1], p3=left_verticies[2],
                       p4=left_verticies[3])
...
with dpg.window(label="tutorial", width=550, height=550):
    with dpg.drawlist(width=width, height=height, tag="d_list"):
        with dpg.draw_layer(tag="main pass", depth_clipping=False, perspective_divide=True,
                            cull_mode=dpg.mvCullMode_Back):
            with dpg.draw_node(tag="cube"):
                dpg.draw_quad(verticies[0], verticies[1], verticies[2], verticies[3],
                              color=[255, 255, 255])  # , fill=colors[1])  # front
                dpg.draw_quad(verticies[0], verticies[4], verticies[5], verticies[1],
                              color=[255, 255, 255])  # , fill=colors[2])  # top
                dpg.draw_quad(verticies[1], verticies[5], verticies[6], verticies[2],
                              color=[255, 255, 255])  # , fill=colors[3])  # right
