...
def load_prameters():
    ...
    config.read('Config\settings.ini', encoding="windows-1252")
    ...
    config.read('Config\chip_params.ini', encoding="utf-8-sig")
...
def save_prameters():
    ...
    with open('Config\settings.ini', mode='w', encoding="windows-1252") as configfile:    # save
        config.write(configfile)

def save_presets():
    np.save('Config\presets_left.npy', context.preset_left)
    np.save('Config\presets_right.npy', context.preset_right)
    np.save('Config\presets_table.npy', context.preset_table)
    np.save('Config\presets.npy', context.preset_all)

def load_presets():
    context.preset_left  = np.load('Config\presets_left.npy', allow_pickle=True)
    context.preset_right = np.load('Config\presets_right.npy', allow_pickle=True)
    context.preset_table = np.load('Config\presets_table.npy', allow_pickle=True)
    context.preset_all   = np.load('Config\presets.npy', allow_pickle=True)
    my_file = Path("Config\spectrum_zero.npy")
    if my_file.is_file():
        context.spectrum_zero = np.load('Config\spectrum_zero.npy', allow_pickle=True)
...
#    context.aim_z_step = [0, 0, 0, 0, 0]
    ...
    while True:
        ...
        if context.aim_steps == 1: # если это первый шаг, то ширину по X оставляем как есть
            ...
            context.aim_y_width[context.aim_steps - 1] = context.aim_z_step[context.aim_steps - 1]*context.aim_z_count[context.aim_steps - 1]
