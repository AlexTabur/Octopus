...
from camera.PixelType_header import *
from camera.scratch import *
# 强制关闭线程
    ...
    def Start_grabbing(self, ui):
        if not self.b_start_grabbing and self.b_open_device:
            ...
            try:
                ...
                self.h_thread_handle = threading.Thread(target=CameraOperation.Work_thread, args=(self, ui))
    ...
    def Work_thread(self, ui):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                n_w, n_h = self.st_frame_info.nWidth, self.st_frame_info.nHeight
                gray = Mono_numpy(a, n_w, n_h)
                val = ui.sb0.value()
                val2 = ui.sb1.value() * 2 + 1
                threshold1 = ui.sb2.value()
                threshold2 = ui.sb3.value()
                rho = 1  # ui.rho.value()
                theta = np.pi / 180 / 10  # ui.theta.value()
                threshold = ui.sb4.value()
                minLineLength = ui.sb5.value() / 100 * n_w * val / 100
                maxLineGap = ui.sb6.value() / 100 * n_w * val / 100
                resized = resize_image(gray, val)
                # resized = cv2.GaussianBlur(resized, (val2, val2), 0)
                min_, max_ = np.min(resized), np.max(resized)
                l = max_ - min_
                resized = np.astype((resized-min_)/l*255, np.uint8)
                resized = cv2.medianBlur(resized, 5)

                img1 = cv2.GaussianBlur(resized, (val2, val2), 2)
                # img1 = resized
                ...
                if lines is not None:
                    if len(lines) > 1:
                        for line in lines:
                            ...
                            # cv2.line(final, (0, int(get_y(0, k, b))), (int(n_w*val), int(get_y(int(n_w*val), k, b))), (0, 255, 0), int(0.1 * val))
                            str_ += f"{np.round(length(line[0]),3)} {round(angle(line[0]), 3)} \n"
                ui.label.setText(str_)
                display_img(ui.label_12, final)
            else:
