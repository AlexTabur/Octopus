...
class PlatformController:
    ...
    def connect_platform(self):
        if not context.platform_connect_thread.is_alive():
            context.platform_connect_thread = Thread(target=context.zplatform.connect, args=[context.plarforms_controller_ip, ], daemon=True)
...
#            self.set_platforms_speed(context.speed_value_platform)
    ...
    def connect_table(self):
        if (not context.table_connect_thread.is_alive()) and (not context.ztable.is_connected):
            context.table_connect_thread = Thread(target=context.ztable.connect, args=[context.table_controller_ip,], daemon=True)
...
#        return
    ...
    def platforms_homing(self):
        ...
        context.gui_hlp.init_platforms_complete(context.break_proc == False)
...
#            else:
            ...
            return context.break_proc == False
